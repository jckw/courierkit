...
for (const [action, quota] of Object.entries(dash.quotas)) {
  console.log(`${quota.name}: ${quota.used}/${quota.limit ?? '∞'}`);
  if (quota.resetsAt) {
...
The engine is stateless, so your database (or billing system) is the source of truth.

- See the [Data Model](/entitlements/concepts/data-model) guide for a concrete schema and query patterns.
